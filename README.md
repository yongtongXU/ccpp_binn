...
- 正常阶段默认使用纯滚动 GBNN：由滚动优化算子展开有限深度候选路径树，并只执行最优分支第一步。
- 滚动优化器会识别开阔水域、岔路查找、障碍边缘、狭窄通道、口袋入口、死区和前沿恢复等局部模式，并按模式切换候选分支权重组。
- 全局条带计划、条带优先规则和候选树条带约束默认关闭，仅在显式启用对应配置时参与规划。
...
```bash
python debug_step_server.py --port 8015
```

然后打开 `http://127.0.0.1:8015`，可以固定在小场景中逐步执行、查看活性场和候选分支，手动强制/新增/纠正规划模式，指定下一步元胞，撤回上一步，并将每一步信息自动保存为 JSON 和 CSV。
