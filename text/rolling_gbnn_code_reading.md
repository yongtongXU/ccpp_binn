...
- `_allowed_next()` 负责基础合法性约束，例如默认过滤普通阶段的斜向移动；`_allowed_for_mode()` 会根据当前规划模式做轻量筛选。
...
实现位置：`RollingOptimizer.classify_planning_state()`。

该函数类似给滚动决策树增加一个局部 SLAM 状态摘要：每次选择下一步前，先根据当前位置周围的可通行邻居、未覆盖邻居、障碍/边界压力、死区状态和未覆盖分支数量识别规划模式。

当前模式包括：

- `open_water`：开阔水域，邻域内未覆盖密度高且障碍压力低。
- `junction_search`：岔路查找，当前位置附近存在多个分离的未覆盖分支。
- `corridor`：狭窄通道，局部可通行连接度低。
- `obstacle_edge`：障碍/边界边缘，周围障碍压力较高。
- `pocket_entry`：口袋入口，只有少量未覆盖出口，容易漏扫或陷入。
- `dead_zone`：死区陷入，当前位置周围无未覆盖邻居但全图仍未完成。
- `frontier_recovery`：前沿恢复，局部已覆盖，需要朝外部未覆盖活性恢复推进。
- `frontier_following`：普通覆盖前沿。

模式识别结果会写入 `decisions.csv` 的 `planning_mode`、`planning_mode_reason`、`mode_uncovered_neighbors`、`mode_traversable_neighbors` 和 `mode_obstacle_pressure` 字段。候选分支记录中也会包含 `planning_mode`，便于回放和调试。

## 9. 分支评分

实现位置：`RollingOptimizer.score_branch()`。

评分先计算基础奖励项和惩罚项，再按 `rolling_optimizer.mode_weights` 中的模式权重组对各项做乘法调节。例如开阔水域更重视航向连续和避免重复，岔路查找更重视结构收益、分支紧迫性和遗漏分支补偿，障碍边缘更重视死区和障碍风险。
